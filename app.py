...
# Separar los datos en dos DataFrames independientes
df_inscripciones = df[df['N_EMPRESA'].isnull()]  # Filtrando los datos de inscripciones
df_empresas = df[df['N_EMPRESA'].notnull()]  # Filtrando los datos de empresas

# Convertir las fechas en el DataFrame de inscripciones
try:
    df_inscripciones['FEC_INSCRIPCION'] = pd.to_datetime(df_inscripciones['FEC_INSCRIPCION'], format='%d/%m/%Y %H:%M:%S', errors='coerce')
    df_inscripciones['FEC_INSCRIPCION'] = pd.to_datetime(df_inscripciones['FEC_INSCRIPCION'], format='%Y-%m-%d %H:%M:%S', errors='coerce')
except Exception as e:
    st.error(f"Error al convertir las fechas: {e}")

# Filtrar solo en la primera pestaña ("Inscripciones")
if page == "Inscripciones":
    ...
    # Aplicar filtros solo en el DataFrame de inscripciones
    if 'N_LOCALIDAD' in df_inscripciones.columns:
    ...
    # Gráficos predefinidos de inscripciones
    ...
    st.write("Datos de empresas:", df_empresas.head())

    if not df_empresas.empty:
        # Gráficos predefinidos de empresas
